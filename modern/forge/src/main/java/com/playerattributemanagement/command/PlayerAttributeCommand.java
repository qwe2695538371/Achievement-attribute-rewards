...
public final class PlayerAttributeCommand {
    ...
    private static int listValues(CommandContext<CommandSourceStack> ctx, ServerPlayer target) {
        ...
        if (extras.isEmpty()) {
            CommandFeedback.sendSuccess(
                ctx.getSource(),
                TextCompat.literal(target.getName().getString() + " has no managed bonuses"),
                false
            );
        ...
        extras.entrySet().stream()
            ...
            .forEach(entry -> CommandFeedback.sendSuccess(
                ctx.getSource(),
                TextCompat.literal(target.getName().getString() + " | " + entry.getKey() + ": " + entry.getValue()),
                false
            ));
        return extras.size();
    }
    ...
    private static int executeAndReport(
        ...
        if (!result.success()) {
            ctx.getSource().sendFailure(TextCompat.literal("Unknown attribute id: " + requestedId));
            return 0;
        }

        ResourceLocation canonical = result.canonicalId();
        switch (operation) {
            case SET -> CommandFeedback.sendSuccess(
                ctx.getSource(),
                TextCompat.literal("Set " + target.getName().getString() + " " + canonical + " extra to " + result.value()),
                true
            );
            case ADD -> CommandFeedback.sendSuccess(
                ctx.getSource(),
                TextCompat.literal("Added " + value + " to " + target.getName().getString() + " " + canonical + ", extra=" + result.value()),
                true
            );
            case RESET -> CommandFeedback.sendSuccess(
                ctx.getSource(),
                TextCompat.literal("Reset " + target.getName().getString() + " " + canonical + " extra"),
                true
            );
            default -> {}
        }
