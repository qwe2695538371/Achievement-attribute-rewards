...
public class AttributeEntryWidget extends ObjectSelectionList.Entry<AttributeEntryWidget> {
    ...
    private static Component resolveName(ResourceLocation id, String rawId, String customName) {
        DisplayNameResolver.ResolvedName resolved = DisplayNameResolver.resolve(
            ...
            attrId -> {
                ...
                Attribute attribute = AttributeResolver.resolveAttribute(attrId, minecraft.level != null ? minecraft.level.registryAccess() : null);
                return attribute != null ? attribute.getDescriptionId() : null;
            },
            ignored -> rawId
        );
        return resolved.translatable()
            ? Component.translatable(resolved.value())
            : Component.literal(resolved.value());
    }
    ...
    public Component getNarration() {
        return Component.translatable("narration.playerattributemanagement.attribute", this.displayName);
    }
    ...
    public void renderTooltip(GuiGraphics guiGraphics, Font font, int mouseX, int mouseY) {
        ...
        tooltip.add(Component.literal("当前值 " + formatNumber(this.snapshot.totalValue())));
        tooltip.add(Component.literal("基础值 " + formatNumber(this.snapshot.baseValue())));
        tooltip.add(Component.literal("附加值 " + formatNumber(this.snapshot.extraValue())));
        String source = resolveSourceLabel(this.attributeId);
        if (source != null) {
            tooltip.add(Component.literal("来源: " + source));
        }
